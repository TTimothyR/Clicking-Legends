...
local function CreateTemplate(boostType, tier, rarity, remainingDuration)
    local clone: ImageButton = potionTemplates:FindFirstChild(tier):Clone();
...
function ItemHandler.UpdateActivePotions()
    ...
    for boostType, tiers in pairs(activePotions) do
        task.spawn(function()
            ...
            local activePotionRarity = items.Potions[tier].Rarity;
            ...
            if itemHolder:FindFirstChild(boostType) then
                ...
                if clone.Tier.Text == tier then
                    ...
                    clone:Destroy();
                    new = CreateTemplate(boostType, tier, activePotionRarity, remainingDuration);
                end
            else
                new = CreateTemplate(boostType, tier, activePotionRarity, remainingDuration);
            end
